...
const Home: React.FC = () => {
  ...
  const SuiteTabItem: React.FC<{ suite: Suite, isActive: boolean, onClick: () => void }> = ({ suite, isActive, onClick }) => {
    ...
    return (
      <button
        ...
        id={`tab-${suite.id}`}
        className={`relative flex-1 flex flex-col items-center gap-3 py-6 md:py-8 px-2 md:px-4 transition-all duration-500 group border-r border-slate-200 dark:border-white/5 last:border-r-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-primary ${
          isActive 
      ...
      >
        <div className={`p-2 md:p-3 rounded-xl md:rounded-2xl transition-all duration-500 ${
          isActive 
  ...
  const StrategicPillar: React.FC<{ icon: any, label: string, desc: string, detail: string, delay: number, isActive: boolean }> = ({ icon: Icon, label, desc, detail, delay, isActive }) => (
    <RevealOnScroll animation="animate-fade-in-up" delay={delay} className="flex-grow">
       <div className={`group relative flex flex-col h-full bg-white dark:bg-brand-surface/20 border rounded-2xl overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-sm dark:shadow-none transform-gpu ${
         isActive 
